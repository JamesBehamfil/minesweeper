...
public class Controller implements Initializable  {
    ...
    /*Phần này dùng để khởi tạo menu lập 'Tự tạo độ khó'*/
    @Override
    ...
    //Tạo màn hình cho chế độ 'Tự tạo độ khó'
    ...
    private void generateGameGrid(int rows, int columns, int mines, String difficulty, int y, int x) {
        ...
        // Kích hoạt cheat bằng cách giữ Shift và nhấn P
    ...
    private void checkTile(Gameboard gboard, Tile tile) {
        if (tile.isOpen() || tile.isFlag()) return;     // Nếu ô đó đã mở hoặc đang đánh dấu cờ, không có gì xảy ra

        revealTile(gboard, tile);   // Hiển thị ô

        if (tile.isMine()) {    // Nếu ô đó có mìn, trò chơi kết thúc và bạn thua!
            gameOver(gboard,false);
            return;
        }

        if (tile.getAdjMines() > 0) {
            if (checkWin(gboard)) gameOver(gboard,true);  // Kiểm tra điều kiện thắng
            return;     // Nếu ô đó có số, kết thúc lệnh
        }
        ...
        // Nếu ô trống, kiểm tra những ô khách có trống không
        for (int y = -1; y < 2; y++) {
            for (int x = -1; x < 2; x++) {
    ...
    //Khi mở những ô có mìn bên cạnh, sẽ hiển thị số mìn xung quang ô đó
    ...
    //Khi mở những ô trống, sẽ mở những ô xung quanh nó. Nếu phát hiện là mìn thì ngưng lại
    ...
    //Đánh dấu ô bằng cờ và dấu chấm hỏi
    private void flagTile(Tile tile) {
        if (tile.isOpen()) return;
        if (!tile.isFlag() && !tile.isQmark()) {
            tile.setImage("/images/flag.png");
            tile.setFlag(true);
            minesLabel.setText(Integer.toString(Integer.parseInt(minesLabel.getText())-1)); // Giảm số mìn hiển thị
        }
        else if (tile.isFlag() && questionMark){
            tile.setImage("/images/qmark.png");
            tile.setFlag(false);
            tile.setQmark(true);
            minesLabel.setText(Integer.toString(Integer.parseInt(minesLabel.getText())+1)); // Tăng số mìn hiển thị
        }
    ...
    //Kiểm tra điều kiện thắng của trò chơi (khi chỉ còn đúng mỗi những ô mìn)
    ...
    //Trò chơi sẽ kết thúc khi chỉ còn lại những ô mìn
    ...
    //Hiển thị màn hình kết thúc trò chơi (Thắng hay thua)
    @FXML
    private void gameOverWindow(Gameboard gboard, boolean win) {
        ...
        if (win) {
            gameover.setTitle("Bạn thắng!");
            l1.setText("Xin chúc mừng! Bạn đã thắng!");
            l1.setTextFill(Color.BLUE);
            msgLabel.setText("BẠN THẮNG!");
            msgLabel.setTextFill(Color.BLUE);
            int score = Integer.parseInt(timeLabel.getText());
            l2.setText("Thời gian của bạn " + score + " giây.");
            switch (gboard.getDifficulty()) {
                case "easy":
            ...
            if (highScore) {
                l3.setText("Kỷ lục mới!");
            }
            else if (!gboard.getDifficulty().equals("custom")) {
                l3.setText("Thời gian tốt nhất: " + bestScore + " giây");
            }
        }
        else {
            gameover.setTitle("Bạn thua!");
            l1.setText("Bạn thua!");
            l1.setTextFill(Color.RED);
            msgLabel.setText("BẠN THUA!");
            msgLabel.setTextFill(Color.RED);
        }
        saveStats();
        pauseButton.setVisible(false);
        msgLabel.setVisible(true);
        Button newGameBtn = new Button("Chơi mới");
        ...
        Button repeatGameBtn = new Button("Xem lại bảng");
    ...
    private void statsWindow(ActionEvent actionEvent) {
        ...
        statistics.setTitle("Chỉ số");
        statistics.initStyle(StageStyle.UTILITY);
        GridPane stats = new GridPane();
        stats.setPadding(new Insets(10, 10, 10, 10));
        stats.setVgap(5);
        stats.setHgap(15);
        stats.add(new Label("Thời gian nhanh nhất"),1,0);
        stats.add(new Label("Thời gian trung bình"),2,0);
        stats.add(new Label("Số lần thắng"),3,0);
        stats.add(new Label("Số lần chơi"),4,0);
        stats.add(new Label("Tỉ lệ thắng"),5,0);
        stats.add(new Label("Dễ:"),0,1);
        stats.add(new Label("Thường:"),0,2);
        stats.add(new Label("Khó:"),0,3);
        Label easy = new Label(scores[0] + " s");
        Label normal = new Label( scores[1] + " s");
        Label hard = new Label(scores[2] + " s");
        stats.add(easy,1,1);
        stats.add(normal,1,2);
        stats.add(hard,1,3);
        DecimalFormat df = new DecimalFormat("0.0");
        stats.add(new Text(df.format(averageTimeEasy) + " s"),2,1);
        stats.add(new Text(df.format(averageTimeNormal) + " s"),2,2);
        stats.add(new Text(df.format(averageTimeHard) + " s"),2,3);
        ...
        resetBtn.setOnAction(e -> {
            scores[0] = 9999;
            easy.setText(scores[0] + " s");
            scores[1] = 9999;
            normal.setText(scores[1] + " s");
            scores[2] = 9999;
            hard.setText(scores[2] + " s");
            saveStats();
        });
        Button closeBtn = new Button ("Đóng");
    ...
    private void aboutWindow(ActionEvent actionEvent) {
        ...
        about.setTitle("Về Dò Mìn");
        about.initStyle(StageStyle.UTILITY);
        Label l1 = new Label("Một phiên bản của game Minesweeper sử dụng Java/JavaFX");
        Application a = new Application() {
            @Override
            public void start(Stage stage) {}
        };
        Label l3 = new Label("© 2024");
        l3.setPadding(new Insets(0,0,5,0));
        Button closeBtn = new Button ("Đóng");
        closeBtn.setPrefWidth(100.0);
        closeBtn.setOnAction(e -> about.close());
        VBox layout = new VBox(3);
        layout.getChildren().addAll(l1,l3,closeBtn);
        layout.setAlignment(Pos.CENTER);
        Scene scene = new Scene(layout,width,height);
    ...
    private void helpWindow(ActionEvent actionEvent) {
        ...
        help.setTitle("Hướng dẫn");
        help.initStyle(StageStyle.UTILITY);
        Label l1 = new Label("Hướng dẫn Dò Mìn");
    ...
    private String loadHelpText() {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(Objects.requireNonNull(getClass().getResourceAsStream("help.txt"))))) {
    ...
    //Lưu lại điểm trên trang 'Chỉ số'
    ...
    //Hiển thị số điểm trên trang 'Chỉ số'
    ...
    /*Khi nhấn tổ hợp giữ Shift + P, sẽ hiển thị vị trí từng quả mìn trong bảng*/
